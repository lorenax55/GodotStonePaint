...
var scene = load("res://Actors/PegatinaT.tscn")
@onready var area: Polygon2D = get_node("../AreaPiedra")
var pegatina = null
var colocar = true
...
@onready var sitio_pegatinas = $"../Pegatinas"

func set_pegatina(new_pegatina):
	brush_mode = BrushModes.STICKER
	pegatina = new_pegatina
...
func _process(_delta):
	...
	is_mouse_in_drawing_area = Geometry2D.is_point_in_polygon(mouse_pos, area.get_polygon())
	...
	if Input.is_mouse_button_pressed(MOUSE_BUTTON_LEFT):
		...
		if check_if_mouse_is_inside_canvas():
			if mouse_pos.distance_to(last_mouse_pos) >= 0.2:
				...
				if brush_mode == BrushModes.PENCIL or brush_mode == BrushModes.ERASER:
					...
					for i in range(5):
						# Generar posiciones aleatorias dentro de una distancia corta de la posición del ratón
						var disperse = 0.25*brush_size
						var random_x = mouse_pos.x + rng.randf_range(-disperse, disperse)
						var random_y = mouse_pos.y + rng.randf_range(-disperse, disperse)

						# Asegurarse de que las posiciones generadas estén dentro de los límites del lienzo
						random_x = clamp(random_x, 0, get_viewport_rect().size.x)
						random_y = clamp(random_y, 0, get_viewport_rect().size.y)

						# Crear una nueva posición del ratón
						var random_mouse_pos = Vector2(random_x, random_y)

						# Agregar el pincel usando la posición aleatoria
						disperse = 0.05
						var og_brush_color = brush_color
						brush_color = brush_color+Color(rng.randf_range(-disperse, disperse),rng.randf_range(-disperse, disperse),rng.randf_range(-disperse, disperse),0)
						add_brush(random_mouse_pos, brush_mode)	
			...
			if brush_mode == BrushModes.STICKER and colocar:
				...
				sitio_pegatinas.add_child(instance)
...
func check_if_mouse_is_inside_canvas():
	...
	if mouse_click_start_pos != null:
		...
		if Geometry2D.is_point_in_polygon(mouse_click_start_pos, area.get_polygon()):	
